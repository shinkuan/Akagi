    ...
    def modify(self, msg: dict):
        ...
        if self.unlock_avartar:
            ...
            if msg['type'] == MsgType.Res:
                ...
                if msg['method'] == '.lq.Lobby.fetchTitleList':
                    action = 'modify'
                    modify_msg['data']['titleList'] = []
                    with open('./id/TitleId.json', 'r', encoding='utf-8') as f:
                        titleId = json.load(f)
                        for id in titleId:
                            modify_msg['data']['titleList'].append(titleId[id]['id'])

                if msg['method'] == '.lq.Lobby.fetchBagInfo':
                    action = 'modify'
                    modify_msg['data']['bag']['items'] = []
                    with open('./id/ItemId.json', 'r', encoding='utf-8') as f:
                        itemId = json.load(f)
                        for id in itemId:
                            if itemId[id]['is_unique'] == 1:
                                modify_msg['data']['bag']['items'].append({
                                    "itemId": itemId[id]['id'],
                                    "stack": 1
                                })
                            else:
                ...
                if msg['method'] == '.lq.Lobby.fetchCharacterInfo':
                    action = 'modify'
                    modify_msg['data']['characters'] = []
                    modify_msg['data']['characterSort'] = []
                    with open('./id/CharacterId.json', 'r', encoding='utf-8') as f:
                        characterId = json.load(f)
                        for id in characterId:
                            modify_msg['data']['characters'].append({
                            ...
                            modify_msg['data']['characterSort'].append(characterId[id]['id'])
                    # Skins
                    modify_msg['data']['skins'] = []
                    with open('./id/SkinId.json', 'r', encoding='utf-8') as f:
                        skinId = json.load(f)
                        for id in skinId:
                            if id in ['400000', '400001']:
                                continue
                            modify_msg['data']['skins'].append(skinId[id]['id'])
                    modify_msg['data']['mainCharacterId'] = self.current_character

                if msg['method'] == '.lq.Lobby.fetchAllCommonViews':
                    action = 'modify'
                    with open('./settings.json', 'r', encoding='utf-8') as f:
                        settings = json.load(f)
                        modify_msg['data']['views'] = [{
                            'values': settings['views'],
                            "index": 0
                        }]
                        modify_msg['data']['use'] = 0
            ...
            if msg['type'] == MsgType.Req:
                if msg['method'] == '.lq.Lobby.changeMainCharacter':
                    ...
                    modify_msg['fake_response'] = {
                        'id': msg['id'],
                        'type': MsgType.Res,
                        'method': '.lq.Lobby.changeMainCharacter',
                        'data': {}
                    }
