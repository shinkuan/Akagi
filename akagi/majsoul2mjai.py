...
from my_logger import both_logger, game_result_log
...
class MajsoulBridge:
    ...
    def input(self, parse_msg: dict) -> dict | None:
        ...
        if ((parse_msg['method'] == '.lq.FastTest.syncGame' or parse_msg['method'] == '.lq.FastTest.enterGame')
            and parse_msg['type'] == GameMessageType.Response):
            self.syncing = True
            reacts = []
            # syncGame_msgs = LiqiProto().parse_syncGame(parse_msg)
            # for msg in syncGame_msgs:
            #     reacts.append(self.input(msg))
            if 'game_restore' not in parse_msg['data']:
                self.syncing = False
                return None
            for action in parse_msg['data']['game_restore']['actions']:
                msg = {
                    'id': -1,
                    'type': GameMessageType.Notify,
                    'method': '.lq.ActionPrototype',
                    'data': action
                }
                both_logger.debug(f"game_restore {self.accountId} received: {msg}")
                reacts.append(self.input(msg))
            self.syncing = False
            if len(reacts)>=1:
                return reacts[-1]
        ...
        # ready
        if parse_msg['method'] == '.lq.FastTest.fetchGamePlayerState' and parse_msg['type'] == GameMessageType.Response:
            # if parse_msg['data']['stateList'] == ['READY', 'READY', 'READY', 'READY']:
            self.AllReady = True
            return None
        # start_game
        if parse_msg['method'] == '.lq.FastTest.authGame' and parse_msg['type'] == GameMessageType.Request:
            self.__init__()
            self.accountId = parse_msg['data']['account_id']
        if parse_msg['method'] == '.lq.FastTest.authGame' and parse_msg['type'] == GameMessageType.Response:
            self.is_3p = len(parse_msg['data']['seat_list']) == 3
            try:
                self.mode_id = parse_msg['data']['game_config']['meta']['mode_id']
            except:
                self.mode_id = -1

            seatList = parse_msg['data']['seat_list']
        ...
        if parse_msg['method'] == '.lq.ActionPrototype':
            ...
            if parse_msg['data']['name'] == 'ActionDiscardTile':
                ...
                if parse_msg['data']['data']['is_liqi']:
